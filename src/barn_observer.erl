%% @author eipugf
%% @doc @todo Add description to barn_observer.


-module(barn_observer).
-behaviour(gen_event).
...
%% ====================================================================
init([]) ->
    error_logger:logfile({open, "barn_log.txt"}),
 	{ok, #state{}}.
...
handle_event(get_grass, State) ->
	error_logger:info_msg("barn recieve message get_grass~n"),
    {ok, State};

handle_event(no_grass, State) ->
	error_logger:info_msg("barn send message no_grass~n"),
    {ok, State};

handle_event(take_grass, State) ->
	error_logger:info_msg("barn send message take_grass~n"),
    {ok, State};

handle_event(save_grass, State) ->
	error_logger:info_msg("barn receive message save_grass~n"),
    {ok, State};
