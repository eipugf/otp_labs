%% @author eipugf
%% @doc @todo Add description to farmer_observer.


-module(farmer_observer).
-behaviour(gen_event).
...
%% ====================================================================
init([]) ->
    error_logger:logfile({open, "farmer_log.txt"}),
 	{ok, #state{}}.
...
-spec handle_event(Event :: term(), State :: term()) -> Result when
	...
	Module2 :: atom().
%% ====================================================================
handle_event(send_get_grass, State) ->
    error_logger:info_msg("farmer send message give_grass~n"),
    {ok, State};

handle_event(recieve_take_grass, State) ->
    error_logger:info_msg("farmer recieve meesage take grass,~n send message save_grass~n"),
    {ok, State};

handle_event(recieve_no_grass, State) ->
	error_logger:info_msg("farmer recieve meesage no grass~n"),
    {ok, State};
