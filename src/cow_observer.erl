%% @author eipugf
%% @doc @todo Add description to cow_observer.
...
%% ====================================================================
init([]) ->
	error_logger:logfile({open, "cow_log.txt"}),
    {ok, #state{}}.
...
handle_event(send_take_milk, State) ->
    error_logger:info_msg("cow send message take_milk~n"),
    {ok, State};

handle_event(recieve_get_milk, State) ->
    error_logger:info_msg("cow receive message get_milk~n"),
    {ok, State};

handle_event(send_no_milk, State) ->
    error_logger:info_msg("cow send message no_milk ~n"),
    {ok, State};

handle_event(receive_no_grass, State) ->
    error_logger:info_msg("cow recieve message no_grass~n"),
    {ok, State};

handle_event(receive_take_grass, State) ->
    error_logger:info_msg("cow receive message take_grass~n"),
    {ok, State};
